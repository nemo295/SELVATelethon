...
async def pyZip(e):
    ...
    password = pass_ if pass_ else "v7x"
...
async def startgmute(event):
    if event.is_private:
        ...
        if user.id == 776886547:
            return await edit_or_reply(event, "**- دي لا يمڪنني كتـم مبرمج السـورس يا عبيط**")
        userid = user.id
    try:
...
async def tmuter(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if not reason:
        return await catevent.edit("**☭ ⦙ رجاء طريقه كتابه الامر خاطئه قم بروئيه قناه شروحات الاوامر : @J0KER_7x**")
...
async def catgban(event):  # sourcery no-metrics
    ...
    await cate.edit(
        f"حظر عام ل : [user](tg://user?id={user.id}) \n {len(san)} من الجروبات التي فيها انت مسؤل"
    )
    ...
    if reason:
        await cate.edit(
            f"[{user.first_name}](tg://user?id={user.id}) تم حظره عام : {count} من الجروبات التي فيها مشرف خلال : {cattaken} ثانيه !!\n**السبب :** `{reason}`"
        )
    else:
        await cate.edit(
            f"[{user.first_name}](tg://user?id={user.id}) تم حظره عام : {count} من الجروبات التي فيها مشرف خلال : {cattaken} ثانيه !!"
...
async def catgban(event):
    ...
    await cate.edit(
        f"تم الغاء الحضر عام : [user](tg://user?id={user.id}) في `{len(san)}` جروب"
    )
    for i in range(sandy):
        try:
    ...
    if reason:
        await cate.edit(
            f"[{user.first_name}](tg://user?id={user.id}) تم الغاء حظر العام في : {count} جرروب خلال :  {cattaken} ثانيه \n**السبب :** `{reason}`"
        )
    else:
        await cate.edit(
            f"[{user.first_name}](tg://user?id={user.id}) تم الغاء حظر العام في : {count} جرروب خلال :  {cattaken} ثانيه"
...
@iqthon.on(admin_cmd(pattern=r"تخزين الجروبات (تشغيل|ايقاف)$"))
...
async def iq(event):
    ...
    if type_of_group == "قناه":
        descript = "☭ ⦙   هذه قناة إختبار أُنشئت بإستعمال تليثون جوكر"
    else:
        descript = "☭ ⦙   هذه المجموعه إختبار أُنشئت بإستعمال تليثون جوكر"
